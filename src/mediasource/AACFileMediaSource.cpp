...
// #define FRAME_MAX_SIZE 10240
//     uint8_t buffer[FRAME_MAX_SIZE] = {0};
//     int buffersize = getFrameFromAACFile(mFd, buffer, FRAME_MAX_SIZE);
//     if(buffersize < 0)
//         return;

//     AACFramePtr aacFrame = std::make_shared<AACFrame>();
//     aacFrame->setFrameBuffer((uint8_t*)&mAdtsHeader, ADTS_HEADER_LENTH, buffer, buffersize);
