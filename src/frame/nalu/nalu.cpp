...
NALUParsing::NALUParsing()
{
    ...
    mBuffer = (uint8_t*)malloc(1024*1024);
    mBufferSize = 0;
}
...
int NALUParsing::inputData(T_NALU_TYPE type, uint8_t *buf, int len, bool clearOldBuffer)
{
    if (clearOldBuffer)
    {
        mBufferSize = 0;
    ...
    memcpy(mBuffer + mBufferSize, buf, len);
    mBufferSize += len;
...
///    因此查找一帧的话，只需要查找四字节的起始码即可。
    ...
    while(1)
    {
        unsigned char* Buf = mBuffer + pos_2;
    ...
    unsigned char* Buf = mBuffer + pos; //这帧数据的起始数据(包含起始码)
...
//        }
    ...
    memmove(mBuffer, mBuffer + pos_2, leftSize);
...
//    fwrite(mBuffer, 1, mBufferSize, fp1);
//    fclose(fp1);
...
//        printf("%s %d nal_unit_type=%d %d %d\n", __FILE__, __LINE__, nal_unit_type, nalu_size, mBufferSize);
    }
